...
def test_sc_model_regular() -> None:
    server = Server(config_path = "tests/testing_server_config.yaml", 
                    handler_exit_function = partial(reply_with_sent_once))
    server.start()
    
    
    client = Client(ConnectionModels.SERVER_CLIENT,
                    config_path="tests/testing_client_config.yaml")
    client.connect()
...
def test_sc_model_mtls() -> None:
    server = Server(config_path = "tests/testing_mtls_server_config.yaml", 
                    handler_exit_function = partial(reply_with_sent_once))
    server.start()
    
    
    client = Client(ConnectionModels.SERVER_CLIENT,
                    config_path="tests/testing_mtls_client_config.yaml")
    client.connect()
